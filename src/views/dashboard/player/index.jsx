...
export default class Player extends Component {
  ...
  togglePlay = () => {
    if (this.audio.paused || this.audio.ended) {
      this.state.isPlay = '暂停'
      this.toPlay()
    } else {
      this.state.isPlay = '播放'
      this.toPause()
    }
  };
  render() {
    ...
    return [
      <div className="player" key="player">
        ...
        <div className="player-btns">
          <button className="pre-btn" onClick={this.preSong} title="上一首"><i className="icon-previous2" /></button>
          <button className="pp-btn" onClick={this.togglePlay} title={isPlay}><i className={ppIcon} /></button>
          <button className="next-icon" onClick={this.nextSong} title="下一首"><i className="icon-next2" /></button>
        </div>
        ...
        <div className="player-mode">
          <i className="icon-heart" title="收藏" />
          <i className={modeIcon} onClick={this.setMode} title={modeTitle} />
          <i className="icon-list" onClick={this.toggleReadyList} title="播放列表" />
        </div>
